...
export default function (options: { publicKey?: string } =  { publicKey: inject('public-key') }) {
    return {
        payWithFlutterwave: function (
            paymentParams: PaymentOptions,
          ): void {
            const payData = {
              ...paymentParams,
              public_key: paymentParams.public_key ?? (options.publicKey ?? inject('public-key')),
        ...
        asyncPayWithFlutterwave: async function (
            ...
            return new Promise((resolve) => {
              const payData = {
                ...paymentData,
                public_key: paymentData.public_key ?? (options.publicKey ?? inject('public-key')),
